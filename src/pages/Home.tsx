...
const Home = () => {
  ...
  const sections = [{
    ...
    color: "sara-pixel3",
    icon: <svg className="w-5 h-5 text-sara-pixel5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    color: "sara-pixel4",
    icon: <svg className="w-5 h-5 text-sara-pixel5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  return <div className="min-h-screen w-full flex flex-col items-center justify-center bg-sara-pixelBg p-4 sm:p-8 overflow-x-hidden relative">
      ...
      <div className="absolute top-40 sm:top-12 left-11 transform -translate-x-1/2 w-11 h-11 pixel-character-container">
        {!imageLoaded && <div className="w-full h-full">
            <Skeleton className="w-full h-full bg-sara-pixel3/20 animate-pulse-gentle" />
          </div>}
        <img src="/lovable-uploads/6ce3c4f5-4273-48e0-82f0-c8022f62c515.png" alt="Pixel Character" className={`w-full h-full object-contain ${imageLoaded ? 'block' : 'hidden'}`} onLoad={() => setImageLoaded(true)} />
      </div>
      
      <div className="relative w-full max-w-4xl z-10 mt-36 sm:mt-40 px-0 py-0 mx-0 my-[46px]">
        {/* Logout button */}
        <div className={`absolute top-2 right-2 z-10 flex gap-2 ${isMobile ? 'scale-75 origin-top-right' : ''}`}>
          <button onClick={handleLogout} className="pixel-button-small p-2 animate-hover transition-all duration-300" aria-label="Log out">
        ...
        <AnimatedWrap animation="fade-in">
          <div className={`text-center mb-6 ${showContent ? 'animate-fade-in' : 'opacity-0'}`}>
            ...
            <p className="text-xl font-press text-sara-pixel4 px-4 animate-fade-in" style={{
            animationDelay: '0.3s'
          }}>
              Welcome 🎉 
            </p>
        ...
        <div className={`grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 ${showContent ? 'animate-fade-in' : 'opacity-0'}`} style={{
        animationDelay: '0.2s'
      }}>
          {sections.map(section => <Link key={section.id} to={`/${section.id}`} className="pixel-tile relative overflow-hidden group">
      ...
      <div className={`mt-6 text-center text-sm text-sara-pixel4 ${showContent ? 'animate-fade-in' : 'opacity-0'}`} style={{
      animationDelay: '0.4s',
      position: 'relative',
      zIndex: 10
    }}>
